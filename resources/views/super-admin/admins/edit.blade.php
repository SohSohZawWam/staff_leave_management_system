...
@section('title', __('super_admin.edit_admin'))

@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="cu-card cu-card-body">
            <h2 class="cu-page-title mb-1">{{ __('super_admin.edit_admin') }}</h2>
            <p class="cu-muted mb-6">{{ app()->getLocale() == 'my' ? $user->name_mm ?? $user->name : $user->name }}</p>

            <form id="admin-edit-form" action="{{ route('super-admin.admins.update', $user) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center gap-4 mt-2">
                        <div id="profile-image-preview"
                            class="w-24 h-24 rounded-full bg-slate-100 border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="cu-input">
                @error('name')
                    <p class="cu-form-error">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input type="text" name="name_mm" id="name_mm" value="{{ old('name_mm', $user->name_mm) }}"
                    class="cu-input">
                @error('name_mm')
                    <p class="cu-form-error">{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" class="cu-input">
                @error('email')
                    <p class="cu-form-error">{{ $message }}</p>
                @enderror
            </div>
            <div>
                <label for="password" class="cu-label">
                    {{ __('common.password') }} <span
                        class="text-slate-500 text-xs font-normal">({{ __('common.leave_blank_hint') }})</span>
                </label>
                <div class="flex justify-center items-center">
                    <input type="password" name="password" id="password" class="cu-input" minlength="8"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <div class="flex items-center">
                    <span
                        class="cu-input w-fit inline-flex items-center px-3 rounded-l-md rounded-r-none border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm whitespace-nowrap">MOST-</span>
                    <input type="text" name="staff_id_number" id="staff_id_number"
                        value="{{ old('staff_id', $user->staff_id) ? preg_replace('/^MOST-/', '', old('staff_id', $user->staff_id)) : '' }}"
                        class="cu-input rounded-l-none border-l-0" placeholder="123 456">
                </div>
                <input type="hidden" name="staff_id" id="staff_id" value="{{ old('staff_id', $user->staff_id) }}">
                @error('staff_id')
                    <p class="cu-form-error">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <select name="position" id="position" class="cu-select">
                    ...
                    @foreach($positions as $en => $mm)
                        <option value="{{ $en }}" {{ old('position', $user->position) == $en ? 'selected' : '' }}>{{ $en }}
                        </option>
                    @endforeach
                </select>
                @error('position')
            ...
            <div>
                ...
                <select name="position_mm" id="position_mm" class="cu-select">
                    <option value="">{{ __('common.select_position') }}</option>
                    @foreach($positions as $en => $mm)
                        <option value="{{ $mm }}" {{ old('position_mm', $user->position_mm) == $mm ? 'selected' : '' }}>{{ $mm }}
                        </option>
                    @endforeach
                </select>
                @error('position_mm')
        ...
        <div class="mb-6">
            <label class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $user->is_active) ? 'checked' : '' }}
                    class="cu-checkbox">
                <span class="text-sm text-slate-700">{{ __('common.active') }}</span>
            </label>
        </div>
        ...
        <div class="mb-6">
            <label class="flex items-center gap-2">
                ...
                <input type="checkbox" name="require_admin_approval" value="1" {{ old('require_admin_approval', $user->require_admin_approval) ? 'checked' : '' }} class="cu-checkbox">
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="cu-btn-primary">{{ __('super_admin.edit_admin') }}</button>
        </div>
        </form>
    </div>
    </div>
