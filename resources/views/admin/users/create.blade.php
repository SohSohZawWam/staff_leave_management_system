...
<div class="max-w-3xl mx-auto">
    <div class="cu-card cu-card-body">
        <h2 class="cu-page-title mb-1">{{ __('admin.create_user') }}</h2>
        <p class="cu-muted mb-6">{{ __('admin.new_staff_account') }}</p>

        <form id="user-create-form" action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="role" id="role" class="cu-select">
                        ...
                        <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>{{ __('common.administrator') }}</option>
                        @endif
                        <option value="department_head" {{ old('role') == 'department_head' ? 'selected' : '' }}>{{ __('common.department_head') }}</option>
                        <option value="staff" {{ old('role') == 'staff' ? 'selected' : '' }}>{{ __('common.staff_role') }}</option>
                    </select>
                    @error('role')
                        <p class="cu-form-error">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="cu-btn-primary">{{ __('admin.create_user') }}</button>
            </div>
        </form>
    </div>
</div>
