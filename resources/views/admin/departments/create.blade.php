...
@section('title', __('admin.create_department'))

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="cu-card cu-card-body">
        <h2 class="cu-page-title mb-1">{{ __('admin.create_department') }}</h2>
        <p class="cu-muted mb-6">{{ __('admin.new_academic_unit') }}</p>
        ...
        <form action="{{ route('admin.departments.store') }}" method="POST">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="cu-btn-primary">{{ __('admin.create_department') }}</button>
            </div>
        </form>
    </div>
</div>
