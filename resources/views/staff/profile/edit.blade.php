...
<div class="max-w-5xl mx-auto">
    <div class="cu-card cu-card-body">
        <h2 class="cu-page-title mb-1">{{ __('staff.profile') }}</h2>
        <p class="cu-muted mb-4">{{ __('staff.profile_subtitle') }}</p>

        <form id="staff-profile-form" action="{{ route('staff.profile.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-3">
                    <h3 class="text-xs font-semibold text-slate-900 uppercase tracking-wide">{{ __('staff.change_password') }}</h3>

                    <div>
                        <label for="current_password" class="cu-label text-xs">{{ __('staff.current_password') }}</label>
                    ...
                    <div>
                        <label for="password" class="cu-label text-xs">{{ __('staff.new_password') }}</label>
                        <div class="flex justify-center items-center">
                    ...
                    <div>
                        <label for="password_confirmation" class="cu-label text-xs">{{ __('staff.confirm_password') }}</label>
                        <div class="flex justify-center items-center">
            ...
            <div class="flex justify-end gap-3 mt-4 pt-3 border-t border-slate-100">
                <a href="{{ route('staff.dashboard') }}" class="cu-btn-secondary text-sm py-1.5 px-4">{{ __('common.cancel') }}</a>
...
@push('scripts')
    <script>
        ...
        document.getElementById('staff-profile-form').addEventListener('submit', function() {
